...
from helper_functions import flatten_list, flatten_list_level1
...
def create_dataset(dataset_dir, instances_dir, background_dir, samples=100, train_val_split=0.8):
    ...
    instances = flatten_list_level1([[(c_index, os.path.join(c_dir, inst)) for inst in os.listdir(c_dir)] for c_index, (c_name, c_dir) in classes.items()])
    ...
    for i in range(samples):
        ...
        if (warped_image.width / warped_bg.width) > 0.5 or (warped_image.height / warped_bg.height) > 0.5:
            scale = max([(warped_image.width / warped_bg.width), (warped_image.height / warped_bg.height)]) * uniform(2, 4)
